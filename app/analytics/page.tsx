...
export default function AnalyticsPage() {
  ...
  if (state.loading) {
    return (
      <ResourceShell title="Analytics" subtitle="Reading saved parcel data.">
  ...
  if (state.error || !state.row) {
    return (
      <ResourceShell title="Analytics" subtitle="Data could not load.">
  ...
  return (
    <ResourceShell title="Analytics" subtitle="Decision analytics reading the saved parcel economics.">
      <ParcelHeader model={model} />

      <Card label="Decision Signal" title={decisionLabel(model.hasCost, model.margin)}>
        <div className="grid gap-3">
          ...
          <Stat label="Route Cost" value={moneyValue(model.routeCost)} gold={model.hasCost} danger={!model.hasCost} />
          ...
          <Stat label="Margin" value={model.hasCost && model.revenue > 0 ? `${model.margin.toFixed(1)}%` : "Not captured"} gold={model.hasTargetMargin} danger={!model.hasTargetMargin} />
        </div>
      </Card>

      <Card label="Price Analytics" title="Price basis">
        <div className="grid gap-3">
          ...
          <Stat label="Effective Selling Price" value={moneyUnit(model.effectivePrice)} gold={model.hasPrice} danger={!model.hasPrice} />
        </div>
      </Card>

      <Card label="Cost Drivers" title="Route cost analysis">
        <div className="grid gap-3">
          <Stat label="Acquisition" value={moneyValue(model.acquisitionTotal)} />
          <Stat label="Logistics" value={moneyValue(model.logisticsTotal)} />
          <Stat label="Processing / Tolling" value={moneyValue(model.processingTotal)} />
