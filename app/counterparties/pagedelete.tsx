...
export default function PartiesPage() {
  ...
  if (state.loading) {
    return (
      <ResourceShell title="Counterparties" subtitle="Reading saved parcel data.">
  ...
  if (state.error || !state.row) {
    return (
      <ResourceShell title="Counterparties" subtitle="Data could not load.">
  ...
  return (
    <ResourceShell title="Counterparties" subtitle="Counterparty control view reading the saved parcel.">
      <ParcelHeader model={model} />

      <Card label="Counterparty State" title={model.hasPositiveSurplus ? "Counterparty Review Ready" : "Counterparty Review Required"}>
        <div className="grid gap-3">
          <Gate title="Supplier / Source" ready={model.stage !== "raw_feedstock" || model.hasCost} note="Raw feedstock routes need verified source, loading point and authority to sell." />
          <Gate title="Buyer / Offtaker" ready={model.hasPrice} note="Buyer price and terms must be confirmed." />
          <Gate title="Transporter / Logistics" ready={model.logisticsUnit > 0 || model.stage !== "raw_feedstock"} note="Transport route, rate and vehicle basis must be verified." />
          <Gate title="Processor / Plant" ready={model.stage !== "raw_feedstock" || model.processingUnit > 0} note="Processing routes need plant/tolling basis." />
        </div>
      </Card>
      ...
      <Card label="Commercial Context" title="Values to verify">
        <div className="grid gap-3">
          ...
          <Stat label="Route Quantity" value={model.routeQty > 0 ? `${model.routeQty.toFixed(3)} units` : "Not captured"} />
          <Stat label="Effective Buyer Price" value={moneyUnit(model.effectivePrice)} gold={model.hasPrice} danger={!model.hasPrice} />
          <Stat label="Route Cost" value={moneyValue(model.routeCost)} />
          <Stat label="Surplus" value={model.hasCost && model.revenue > 0 ? money(model.surplus) : "Not captured"} gold={model.hasPositiveSurplus} danger={!model.hasPositiveSurplus} />
        </div>
      </Card>
