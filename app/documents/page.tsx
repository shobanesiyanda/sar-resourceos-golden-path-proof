...
export default function DocumentsPage() {
  ...
  if (state.loading) {
    return (
      <ResourceShell title="Documents" subtitle="Reading saved parcel data.">
  ...
  if (state.error || !state.row) {
    return (
      <ResourceShell title="Documents" subtitle="Data could not load.">
  ...
  return (
    <ResourceShell title="Documents" subtitle="Document readiness view reading the saved parcel.">
      <ParcelHeader model={model} />

      <Card label="Document Readiness" title={model.hasPositiveSurplus ? "Document Pack Review Ready" : "Documents Not Ready"}>
        <div className="grid gap-3">
          <Gate title="Commodity evidence" ready={model.hasCommodity} note="Commodity, resource and material must be captured." />
          <Gate title="Quantity evidence" ready={model.hasQuantity} note="Product and route quantity must be captured." />
          <Gate title="Buyer price evidence" ready={model.hasPrice} note="Buyer price, PO or written confirmation must support the model." />
          <Gate title="Route cost evidence" ready={model.hasCost} note="Supplier, logistics, processing and verification costs must be supported." />
      ...
      <Card label="Required Documents" title="Release-critical pack">
        <div className="grid gap-3">
          <Stat label="Supplier / Source Confirmation" value="Required" danger />
          <Stat label="Buyer / Offtake Confirmation" value="Required" danger />
          <Stat label="Transport / Logistics Quote" value="Required" danger />
          <Stat label="Plant / Processor Quote" value={model.stage === "raw_feedstock" ? "Required" : "Route-specific"} danger={model.stage === "raw_feedstock"} />
          <Stat label="Assay / Quality Evidence" value="Required" danger />
        </div>
      </Card>
