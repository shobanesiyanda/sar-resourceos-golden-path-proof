...
function openCount(rows: DocumentRow[]) {
  return rows.filter((r) => r.status === "Pending" || r.status === "Blocked")
    .length;
}

function completeCount(rows: DocumentRow[]) {
  return rows.filter((r) => r.status === "Complete" || r.status === "Approved")
    .length;
}
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
      ...
      <p className="mt-3 text-sm leading-6 text-slate-400">
        {doc.notes || "No document note captured yet."}
      </p>
    </div>
  );
}
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: docs, error: docsError } = await supabase
        .from("documents")
        .select("id, document_name, document_type, status, notes")
      ...
      <ResourceShell title="Documents Control" subtitle="Documents module error">
        <Card label="Error" title="Could not load documents">
          <p className="mt-3 text-red-200">
            {error || "Could not load documents page."}
          </p>
        </Card>
      </ResourceShell>
    );
    ...
    >
      ...
      <Card label="Control Rule" title="Documents block release until complete">
        <div className="mt-5 rounded-2xl border border-red-400/30 bg-red-500/10 p-4">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-300">
            ...
            buyer/offtake support, assay records and movement evidence must be
            complete before dispatch or finance handoff.
          </p>
        </div>
      </Card>
      ...
      <Card label="Next Document Phase" title="Storage upload comes later">
        <p className="mt-3 text-sm leading-7 text-slate-300">
          ...
          phase will add Supabase Storage uploads, signed document viewing,
          versioning, approval workflow and audit events.
        </p>
