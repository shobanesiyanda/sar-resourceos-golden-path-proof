...
export default function OperationsPage() {
  ...
  return (
    <ResourceShell
      title="Operations"
      subtitle="Commodity-specific execution readiness reading the saved parcel."
    >
      <ParcelHeader model={model} />

      <Card label="Operations State" title={model.hasTargetMargin ? "Ready for Route Review" : "Not Ready for Dispatch"}>
        <div className="grid gap-3">
          <Gate title="Commodity and material captured" ready={model.hasCommodity} note="Resource and material must be selected before operations can proceed." />
          <Gate title="Quantity captured" ready={model.hasQuantity} note="Product and route quantity must be available." />
          <Gate title="Buyer / offtake price captured" ready={model.hasPrice} note="Effective selling price must be captured." />
          <Gate title="Route cost captured" ready={model.hasCost} note="Route cost must be captured before dispatch planning." />
          <Gate title="Target margin reached" ready={model.hasTargetMargin} note="Target margin is 18% or higher before release review." />
        </div>
      </Card>

      <Card label="Movement Basis" title="Commodity-specific operation basis">
        <div className="grid gap-3">
          <Stat label="Product Quantity" value={model.productQty > 0 ? `${model.productQty.toFixed(3)} units` : "Not captured"} />
          <Stat label={model.stage === "raw_feedstock" ? "Feedstock / Route Quantity" : "Route Quantity"} value={model.routeQty > 0 ? `${model.routeQty.toFixed(3)} units` : "Not captured"} gold={model.routeQty > 0} />
          <Stat label="Effective Buyer / Offtake Price" value={moneyUnit(model.effectivePrice)} />
          <Stat label="Route Cost" value={moneyValue(model.routeCost)} />
        </div>
      </Card>

      <Card label="Missing Capture" title="Operations fields still required">
        <div className="grid gap-3">
          <Stat label="Origin / Source / Loading Point" value="Not yet captured" danger />
          <Stat label={terms.storageLabel} value="Not yet captured" danger />
          <Stat label={terms.processingLabel} value={terms.processingValue} danger={terms.processingRequired} note={terms.processingNote} />
          <Stat label="Buyer / Offtake Delivery Point" value="Not yet captured" danger />
          <Stat label="Transporter / Vehicle / Logistics" value="Not yet captured" danger />
          <Stat label={terms.qualityLabel} value="Not yet captured" danger note={terms.qualityNote} />
        </div>
      </Card>
