...
export default function OperationsPage() {
  ...
  if (state.loading) {
    return (
      <ResourceShell title="Operations" subtitle="Reading saved parcel data.">
  ...
  if (state.error || !state.row) {
    return (
      <ResourceShell title="Operations" subtitle="Data could not load.">
  ...
  return (
    <ResourceShell title="Operations" subtitle="Execution readiness view reading the saved parcel.">
      ...
      <Card label="Operations State" title={model.hasTargetMargin ? "Ready for Route Review" : "Not Ready for Dispatch"}>
        <div className="grid gap-3">
          ...
          <Gate title="Buyer price captured" ready={model.hasPrice} note="Effective selling price must be captured." />
      ...
      <Card label="Movement Basis" title="Operational quantities">
        <div className="grid gap-3">
          ...
          <Stat label="Effective Price" value={moneyUnit(model.effectivePrice)} />
          <Stat label="Route Cost" value={moneyValue(model.routeCost)} />
        </div>
      </Card>

      <Card label="Missing Capture" title="Route fields still required">
        <div className="grid gap-3">
          <Stat label="Origin / Loading Point" value="Not yet captured" danger />
          <Stat label="Plant / Processing Point" value={model.stage === "raw_feedstock" ? "Required" : "Optional"} danger={model.stage === "raw_feedstock"} />
          <Stat label="Buyer / Delivery Point" value="Not yet captured" danger />
          <Stat label="Transporter / Truck" value="Not yet captured" danger />
        </div>
      </Card>
