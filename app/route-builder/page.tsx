...
export default function RouteBuilderPage() {
  ...
  return (
    <ResourceShell
      title="Route Builder"
      subtitle="Commodity-specific route builder reading the saved parcel economics."
    >
      ...
      <Card label="Route State" title={model.hasTargetMargin ? "Route Review Ready" : "Route Incomplete"}>
        <div className="grid gap-3">
          ...
          <Gate title="Buyer / offtake price captured" ready={model.hasPrice} note="Market/reference or negotiated buyer price must be captured." />
      ...
      <Card label="Route Cost Build-Up" title="Commodity route economics">
        <div className="grid gap-3">
          <Stat label="Acquisition / Source Cost" value={moneyValue(model.acquisitionTotal)} note={`${moneyUnit(model.acquisitionUnit)} × ${model.routeQty.toFixed(3)}`} />
          <Stat label="Transport / Logistics / Handling" value={moneyValue(model.logisticsTotal)} note={`${moneyUnit(model.logisticsUnit)} × ${model.routeQty.toFixed(3)}`} />
          <Stat label={terms.processingLabel} value={moneyValue(model.processingTotal)} note={terms.processingNote} />
          <Stat label={terms.qualityLabel} value={moneyValue(model.verificationCost)} note={terms.qualityNote} />
          <Stat label="Total Route Cost" value={moneyValue(model.routeCost)} gold={model.hasCost} danger={!model.hasCost} />
        </div>
      </Card>

      <Card label="Route Capture Needed" title="Commodity-specific next fields">
        <div className="grid gap-3">
          ...
          <Stat label="Transporter / Logistics" value="Not yet captured" danger />
          <Stat label={terms.deliveryLabel} value="Not yet captured" danger />
