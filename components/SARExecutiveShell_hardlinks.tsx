...
function slugifyNav(item: string) {
  return item
    .trim()
    .toLowerCase()
    .replace(/&/g, "and")
...
function navHref(systemCode: SARExecutiveShellProps["systemCode"], item: string) {
  const key = item.trim().toLowerCase();

  if (key === "dashboard" || key === "dash") return "/";
  ...
  const accountingRoutes: Record<string, string> = {
    ...
    tax: "/accounting/tax",
    vat: "/accounting/tax",
    "trade finance": "/accounting/trade-finance",
    reports: "/accounting/reports",
    approvals: "/accounting/approvals",
    admin: "/accounting/admin",
  };

  const intelligenceRoutes: Record<string, string> = {
    "market intelligence": "/intelligence/market",
    market: "/intelligence/market",
    "route intelligence": "/intelligence/routes",
    routes: "/intelligence/routes",
    risk: "/intelligence/risk",
    "ai recommendations": "/intelligence/recommendations",
  ...
  if (systemCode === "BUSINESSOS") return businessRoutes[key] ?? `/${slugifyNav(item)}`;
  if (systemCode === "RESOURCEOS") return resourceRoutes[key] ?? `/${slugifyNav(item)}`;
  if (systemCode === "ACCOUNTINGOS") return accountingRoutes[key] ?? `/accounting/${slugifyNav(item)}`;
  if (systemCode === "INTELLIGENCE") return intelligenceRoutes[key] ?? `/intelligence/${slugifyNav(item)}`;

  return `/${slugifyNav(item)}`;
}
...
export function SARExecutiveShell({
  ...
  const adminHref =
    systemCode === "ACCOUNTINGOS"
      ? "/accounting/admin"
      : systemCode === "INTELLIGENCE"
      ? "/intelligence/admin"
      : "/admin";
    ...
    <main className="relative min-h-screen bg-[#050608] text-white">
      <div className="relative z-10 flex min-h-screen overflow-hidden">
        <aside className="hidden w-[238px] shrink-0 border-r border-[#27210f] bg-[#080a0f] lg:flex lg:flex-col">
          <HardLink href="/" className="block border-b border-[#27210f] px-5 py-4">
        ...
        <section className="flex min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-30 border-b border-[#27210f] bg-[#080a0f]/95 backdrop-blur">
            <div className="flex h-[52px] items-center gap-2 px-3 sm:px-4 lg:h-[68px] lg:gap-3 lg:px-6">
              ...
              <HardLink href="/" className="min-w-0 shrink-0">
              ...
              <div className="ml-auto flex items-center gap-1.5 lg:gap-2">
                <HardLink href="/notifications" className="block">
                  <TopbarIcon label="7" />
                </HardLink>
                <HardLink href="/tasks" className="block">
                  <TopbarIcon label="12" />
                </HardLink>
            ...
            {rightPanel ? (
              <HardLink href="/reports?view=ai-recommendations" className="block border-t border-[#141a24] px-3 py-1.5 xl:hidden">
