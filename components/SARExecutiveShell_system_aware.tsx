...
function navHref(systemCode: SARExecutiveShellProps["systemCode"], item: string) {
  ...
  if (systemCode === "BUSINESSOS") {
    return businessRoutes[key] ?? `/${slugifyNav(item)}`;
  }

  if (systemCode === "RESOURCEOS") {
    return resourceRoutes[key] ?? `/${slugifyNav(item)}`;
  }

  if (systemCode === "ACCOUNTINGOS") {
    return accountingRoutes[key] ?? `/accounting/${slugifyNav(item)}`;
  }

  if (systemCode === "INTELLIGENCE") {
    return intelligenceRoutes[key] ?? `/intelligence/${slugifyNav(item)}`;
  }
          ...
          <div className="flex min-h-0 flex-1 overflow-hidden">
            <div className="min-w-0 flex-1 overflow-y-auto px-3 py-2.5 sm:px-4 lg:px-5 lg:py-4">
              <div className="mb-3 flex flex-col gap-2 border-b border-[#1d2430] pb-3 lg:flex-row lg:items-end lg:justify-between">
                ...
                <Link
                  href={systemCode === "ACCOUNTINGOS" ? "/accounting/admin" : systemCode === "INTELLIGENCE" ? "/intelligence/admin" : "/admin"}
